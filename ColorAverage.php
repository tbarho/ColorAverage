...
*/
		...
		$totalHeight = $blockHeight * $yCount;
		$totalWidth = $blockWidth * $xCount;
		
		$palette = '<div style="width:'. $totalWidth . 'px; height: ' . $totalHeight . 'px; float: left;">';
		
		foreach($colorArray as $v1) {
			foreach($v1 as $v2) {
				$palette .= '<div style="background: #' . $v2['rgbHex'] . '; width: ' . $blockWidth . 'px; height: ' . $blockHeight . 'px; float: left;">&nbsp;</div>';
			}
		}
